...
3. "Render Properties" > "Film" > "Pixel Filter" > Set "Type" to "Box"
...
PALETTE_FILE_PATH = (
    "I:/Programming/Cortex-Command-Mods/blender_test.rte/Blender Files/palette.bmp"
)
...
def convert(self):
    ...
    os.remove(
        old_path
    )  # old_path's image can't be overwritten as the old images start at frame 1, instead of frame 0
...
def get_img_paths():
    ...
    new_path = old_path_with_hashtags_for_framecount.replace(
        "###", str(frame - 1).zfill(3)
    )
...
def process_img(old_path, new_path, palette):
    ...
    # old_img.convert("RGB").save(new_path)

    # CC's background color is #FF00FF
    new_img = Image.new("RGBA", old_img.size, (255, 0, 255, 255))
    new_img.paste(old_img, (0, 0), old_img)

    # TODO: A major issue is that we'll always dither the background color as well
    # TODO: The quantize method unfortunately doesn't work on RGBA,
    # TODO: so replace it with one that *does* work on RGBA (import bisqwit's algorithm?)
    new_img.convert("RGB").quantize(palette=palette_img, dither=DITHERING).save(
        new_path
    )
